...
2. 2. [Create an API key](https://www.crawlvolt.com/en/console/api-keys) and save it when shown.
   3. 3. Set the key in your terminal. In Bash or zsh, the following prompt hides the key while you paste it:
      ...
      4. ```bash
         ...
         | Schema-based product extraction (paused) | [extract_product.mjs](extract_product.mjs) | Not available in the current beta |
         ...
         The `extract_product.mjs` script is paused and exits before making an API request. CrawlVolt currently returns page content as Markdown; it does not promise a native `retailer`/`product`/`price` JSON schema or reliable product fields on arbitrary sites. Inspect the Markdown first, then parse and validate the fields required by your own workflow.
